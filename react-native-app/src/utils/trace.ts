import type { TraceStep, RcaResult, Diagnostics } from '../types/dashboard';

export function getTraceDetail(step: TraceStep): string[] {
  const results = step?.output?.results ?? [];
  if (step.tool === 'ticket_search') {
    return results.map(
      (r) => `${r.ticket_id} (${(r.similarity * 100).toFixed(0)}% match) - ${r.failure_mode}, ${r.severity} - "${r.symptom.slice(0, 70)}..."`
    );
  }
...
export function getProviderMeta(data: RcaResult) {
  ...
  return { provider, label, className };
}
